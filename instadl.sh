...
curl -s --user-agent "Mozilla/5.0 (Windows; U; Windows NT 5.1; en-US; rv:1.7.5) Gecko/20060127 Netscap
e/8.1" https://www.instagram.com/$2/ | grep -e '"profile_pic_url_hd":' | tr ',' '\n' | grep -e .jpg | grep 's320x320' | awk 'NR==1' | sed 's,", ,g' | awk '{print $3}' | while read url;do $(wget -O $2.jpg $url 2> /dev/null);done
